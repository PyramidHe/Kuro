...
def pv3d_line_index(proj_matrices, points3d, vecs3d, num, resolution):
    """
...
class Bottleneck_(nn.Module):
    ...
    def __init__(self, num_points, channel_dim=380):
        ...
        self.b0 = ConvBN(channel_dim * num_points, channel_dim)
        self.b1 = ConvBN(channel_dim, 256)
...
import numpy as np
from utils.file_io import read_points_file, read_pv_file, scanner_converter, read_img, read_cam
import glob
import os
...
if __name__=="__main__":
    ...
    downsample = 1.0
    cam_folder = "/home/flexsight/Downloads/dtu/Cameras"
    img_folder = "/home/flexsight/Downloads/dtu/Rectified/scan2_train"
    pv_file = "/home/flexsight/Downloads/dtu/GT/scan2_train.txt"
    ...
    cam_list = sorted(glob.glob(os.path.join(cam_folder, "*.txt")))
    #points, vecs, nocam = read_points_file(point_file, mode="Train")
    points, vecs, scalar, nocam = read_pv_file(pv_file)

    true_points = points + np.expand_dims(scalar, axis=-1)*vecs
